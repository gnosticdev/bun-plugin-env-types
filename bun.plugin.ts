...
export default function bunEnvPlugin(
	opts: PluginOpts = { dtsFile: 'env.d.ts', glob: '.env*' },
): BunPlugin {
	return {
		name: 'bun-plugin-env',
		setup: async () => {
			...
			await createEnv({ typeDefs: typeDefinitions })
...
async function createEnv({
	typeDefs,
	envFile,
}: { typeDefs: Set<string>; envFile?: string }) {
	const envDtsFile = envFile ?? path.join(process.cwd(), 'env.d.ts')
	const timestamp = new Date().toLocaleString()
	const dtsContent = generateDtsString(typeDefs, timestamp)
...
function generateDtsString(typeDefs: Set<string>, timestamp: string) {
	...
	const dtsContent = `// env.d.ts - Generated by Bun plugin at ${timestamp}
